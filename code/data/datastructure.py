...
class RetinaModule:
    def __init__(self, cfg):
        ...
        self.train_files = self._collect_pairs(split="train")
        self.val_files   = self._collect_pairs(split="val")

    def _collect_pairs(self, split="train"):
        ...
        if len(files) == 0:
            raise RuntimeError(f"No retina data found in {image_dir}")
    ...
    def _get_transforms(self):
        return Compose([
            ...
            Resized(keys=["image", "label"], spatial_size=self.image_size,
    ...
    def get_loader(self, split="train"):
        ...
        dataset = Dataset(files, transform=self._get_transforms())
